...
if(params$dataFile == ""){
  ...
  rmdParams$dataFile <- path.expand("~/Dropbox/Home/trapping/2025-01-26-telford.csv") # not open data
} else {
...
Figure \@ref(fig:speciesRecordedTimelineInclNone) repeats this plot but includes the total number of traps recording no species caught per month.

```{r speciesRecordedTimelineInclNone, fig.cap="Frequency of species caught per month - including None"}
p <- ggplot2::ggplot(plotDT[Species != "Unspecified"], aes(x = as.Date(Month), y = n, colour = Species)) +
  geom_point() +
  geom_smooth() +
  labs(x = "Month", y = "N caught per month (including 'None')") +
      theme(legend.position = "bottom",
          legend.direction = "horizontal") +
  labs(x = "Month")
